...
Take a sequence S[1…n] over an alphabet Σ of size σ.
At the root, partition the alphabet into two halves (e.g., low vs. high symbols).
Store a bitmap that tells whether each symbol in S belongs to the left or right half.
Recursively build the same structure for the two subsequences.
The result is a balanced binary tree of height ⌈log₂ σ⌉.
...
Wikipedia: Wavelet Tree — Succinct overview, access algorithm, and extensions.
ScienceDirect Survey: Navarro (2012) — Wavelet Trees for All — The definitive modern survey (§1–3 cover structure, compression, and tree-shape variants).
License
